...
fn bench_planar_generation(n: usize) -> impl Fn(&mut Bencher) {
    move |b| {
        b.iter(|| {
            generate(black_box(n), black_box(Some(44))).to_pet_graph();
        })
    }
}
...
fn bench_er_generation(n: usize) -> impl Fn(&mut Bencher) {
    move |b| {
        b.iter(|| {
            generate_petgraph(black_box(n), black_box(0.5), black_box(Some(44)));
        })
    }
}
...
fn generation_benchmark(c: &mut Criterion) {
    ...
    pgg.bench_function(
        "Planar Generation (|G|=1000)",
        bench_planar_generation(1000),
    );
    drop(pgg);
    ...
    erg.bench_function(
        "Erdos Renyi Generation (|G|=1000)",
        bench_er_generation(1000),
    );
}
